...
class Plugin extends \WP_Widget {
    ...
    function plugin_admin_menu() {
        ...
        foreach($this->menu_pages as $page => $opt) {
            ...
            if(is_array($opt)) {
                ...
                if(empty($opt['func'])) {
                    $opt['func'] = $page;
                } elseif(is_string($opt['func'])) {
                    $opt['func'] = array($this, $opt['func']);
                }
                add_menu_page($page, $page, $capability, $slug, $opt['func'], $icon, $position);
                ...
                if (!empty($opt['sub']) && is_array($opt['sub'])) {
                    foreach ($opt['sub'] as $sub => $op) {
                        if(! is_numeric($sub)) {
                            ...
                            add_submenu_page($slug, $sub, $sub, $sub_capability, self::toSlug($page . '-' . $sub), $op['func']);
                        } else {
    ...
    function render($file = '', $params = array(), $output = true, $folder = 'admin') {
        if (!empty($file)) {
            $filename = $file . '.php';
            $filepath = $this->plugin_base . '/views/' . (! empty($folder) ? $folder . '/' : '');
            ...
            if (file_exists($filefull)) {
                ...
                $message = 'File "/views/' . (! empty($folder) ? $folder . '/' : '') . $filename . '" does not exist!';
